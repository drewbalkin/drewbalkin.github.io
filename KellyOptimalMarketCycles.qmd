...
Secondly, all these portfolio optimization methods have the problem that one needs to know the true distribution of future returns, and they are sensitive to perturbations of the underlying distribution.
...
with torch.no_grad():
    ...
    ax[0,0].set_title('Kelly Strategy (yellow) vs 100% Stock (blue)');
...
with torch.no_grad():
    ...
    TR = np.array(TR)
    TR = TR/TR[0]
    ax[0,0].set_title('PE timing (yellow) vs CAPE timing (cyan)')
    ax[0,0].plot(t[index_start:],x_plt[index_start:]/x_plt[index_start],'y',
...
```{python}
plt.figure(1,(8,5.5));
plt.title('Wealth processes under PE timing (yellow) vs CAPE timing (cyan) and 2x leveraged (blue)');
